...
const store = createStore({
	state() {
		return {
			...
			nationalitySelected: 'Все страны',
			genderSelected: 'Без разницы',
			positionSelected: 'Все должности',
			tdChecked: false,
			gphChecked: false,
			smzChecked: false,
	...
	actions: {
		clearFilters({ commit, state }) {
			commit('SET_NATIONALITY_SELECTED', 'Все страны')
			commit('SET_GENDER_SELECTED', 'Без разницы')
			commit('SET_POSITION_SELECTED', 'Все должности')
		...
		filterEmployees({ commit, state }) {
			...
			if (state.nationalitySelected !== 'Все страны') {
				filteredEmployees = filteredEmployees.filter(
					e => e.country_full === state.nationalitySelected
				)
			}
			if (state.genderSelected !== 'Без разницы') {
				filteredEmployees = filteredEmployees.filter(
					e => e.gender === state.genderSelected
				)
			}
			if (state.positionSelected !== 'Все должности') {
				filteredEmployees = filteredEmployees.filter(
